...
class ItemValidationTest(FunctionalTest):
    ...
    def test_cannot_save_duplicate_todos(self):

        self.browser.get(self.live_server_url)
        self.get_item_input_box().send_keys('Buy feathers')
        self.get_item_input_box().send_keys(Keys.ENTER)
        self.wait_for_row_in_list_table('1: Buy feathers')
        # Eith will try to save the save to-do item
        self.get_item_input_box().send_keys('Buy feathers')
        self.get_item_input_box().send_keys(Keys.ENTER)
        # There should be an error on the page
        self.wait_for(lambda: self.assertEqual(
            self.browser.find_element(By.CSS_SELECTOR, '.has-error').text,
            "Already got this in your list"
        ))
